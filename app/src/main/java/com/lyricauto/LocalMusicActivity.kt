...
class LocalMusicActivity : AppCompatActivity() {
    ...
    private val storagePermissionLauncher = registerForActivityResult(
        ...
        if (isGranted) {
            ...
            Toast.makeText(this, "需要存储权限才能扫描本地音乐", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun setupRecyclerView() {
        adapter = LocalMusicAdapter(musicList) { music ->
            searchLyricForMusic(music)
        }
    ...
    private fun scanLocalMusic() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                if (folderPath != null) {
                    selection = "${android.provider.MediaStore.Audio.Media.IS_MUSIC} = ? AND ${android.provider.MediaStore.Audio.Media.TITLE} IS NOT NULL AND ${android.provider.MediaStore.Audio.Media.DATA} LIKE ?"
                    selectionArgs = arrayOf("1", "$folderPath%")
                } else {
                    selection = "${android.provider.MediaStore.Audio.Media.IS_MUSIC} = ? AND ${android.provider.MediaStore.Audio.Media.TITLE} IS NOT NULL"
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (musicList.isEmpty()) {
                        ...
                        Toast.makeText(this@LocalMusicActivity, getString(R.string.found_songs, musicList.size), Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
                withContext(Dispatchers.Main) {
                    binding.scanBtn.isEnabled = true
                    binding.scanStatusText.text = "扫描失败"
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                if (lyric != null && !lyric.isEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@LocalMusicActivity, R.string.lyric_download_failed, Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@LocalMusicActivity, "搜索失败", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
