        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            when (intent?.action) {
                com.lyricauto.service.LyricDownloadService.ACTION_LYRIC_DOWNLOADED -> {
                    ...
                    val title = intent.getStringExtra(com.lyricauto.service.LyricDownloadService.EXTRA_TITLE) ?: ""
                    val artist = intent.getStringExtra(com.lyricauto.service.LyricDownloadService.EXTRA_ARTIST) ?: ""
    ...
    private fun previewLyric(result: LyricSearchResult) {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                e.printStackTrace()
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@LyricSearchActivity, "预览失败", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                if (lyric != null && !lyric.isEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@LyricSearchActivity, "下载失败", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
